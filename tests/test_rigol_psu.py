...
def mock_psu():
    with patch('pyvisa.ResourceManager'):
        driver = RigolDP832("TCPIP::1.2.3.12::INSTR")
...
def test_set_voltage_specific_channel(mock_psu):
    ...
    mock_psu.inst.write.assert_any_call("SOUR2:VOLT 12.0")
...
def test_get_voltage(mock_psu):
    ...
    assert val == 3.3
    mock_psu.inst.query.assert_any_call("SOUR1:VOLT?")


def test_set_current_limit(mock_psu):
    mock_psu.set_current_limit(1.0, channel=3)
    mock_psu.inst.write.assert_any_call("SOUR3:CURR 1.0")
...
def test_set_output_on_off(mock_psu):
    ...
    mock_psu.inst.write.assert_any_call("OUTP CH2,OFF")
...
def test_set_ovp_enables_protection(mock_psu):
    ...
    mock_psu.inst.write.assert_any_call("SOUR1:VOLT:PROT 30.0")
    mock_psu.inst.write.assert_any_call("SOUR1:VOLT:PROT:STAT ON")


def test_set_ocp_enables_protection(mock_psu):
    mock_psu.set_ocp(2.0, channel=1)
    mock_psu.inst.write.assert_any_call("SOUR1:CURR:PROT 2.0")
    mock_psu.inst.write.assert_any_call("SOUR1:CURR:PROT:STAT ON")


def test_clear_protection(mock_psu):
    mock_psu.clear_protection(channel=1)
    mock_psu.inst.write.assert_any_call("SOUR1:VOLT:PROT:CLE")
    mock_psu.inst.write.assert_any_call("SOUR1:CURR:PROT:CLE")
...
def test_set_tracking_mode(mock_psu):
    mock_psu.set_tracking_mode(True, channel=2)
    mock_psu.inst.write.assert_any_call("OUTP:TRAC CH2,ON")


def test_save_load_state_valid_range(mock_psu):
    mock_psu.save_state(5)
    mock_psu.inst.write.assert_any_call("*SAV 5")
    mock_psu.load_state(10)
    mock_psu.inst.write.assert_any_call("*RCL 10")


def test_save_state_out_of_range_raises(mock_psu):
    with pytest.raises(ValueError):
        mock_psu.save_state(11)
    with pytest.raises(ValueError):
